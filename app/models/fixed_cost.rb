class FixedCost < ActiveRecord::Base
  ...
  belongs_to :project
  validates :reason, :presence=>true, :if=>:check_date_changed?, :on=>:update
  ...
  def check_date_changed?
    unless self.milestone_start_date_was.blank? &&  self.milestone_end_date_was.blank?
      if (self.milestone_start_date_changed? || self.milestone_end_date_changed? || self.reason_changed?)
        return true 
      else
        return false
      end
  ...
  def update_milestone_date(change_flag, date)
    ...
    puts "change_days",change_days
    fixed_costs = self.project.fixed_costs.where("id > #{self.id}")
    unless fixed_costs.blank?
